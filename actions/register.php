...
if (isset($_GET["error"])) {
	...
	if (isset($resp->access_token)) {
        ...
        if (REQUIRE_IN_GUILD_LOGIN == false) {
			$USR_SERVER = true;
		}

		if($USR_SERVER == false) {
			header("Location: ../index.php?notInDiscord");
		} else {
			try{
...
function setupAndSendOnline(){
	...
	// UPDATE USERS INFO
    $stmt = $pdo->prepare("UPDATE users SET name=?, avatar=?, identifier=? WHERE discordid=?");
    $stmt->execute([$USR_name, $USR_avatar, $guildMember->nick, $USR_discordid]);
